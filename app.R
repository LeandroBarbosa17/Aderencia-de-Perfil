...
ui <- fluidPage(
    ...
    # Application title
    titlePanel("Aderencia de Perfil para Carreira de Cientista de Dados"),
    ...
    fluidRow(
      ...
      column(6, actionButton("Processar", "Processar"), helpText("A linha vermelha mostra a sua aderencia a profissão de Cientista de Dados"), plotOutput("GrafResultado") )
...
server <- function(input, output) {
  ...
  observeEvent(input$Processar, {
    ...
    inferencia <- fuzzy_inference(sistema, list(Gosta_exatas = input$sexatas, Rel_interpess = input$sinter, Escre_codigo = input$scodigo, Pefil_lider = input$slider, Gosta_Estudar = input$sestudar, 
